...
"""Tests for fastsafetensors.auto_loader module."""
...
from fastsafetensors.auto_loader import _LOADER_REGISTRY, AutoLoader
...
class TestAutoLoaderSignature:
    """Test AutoLoader constructor signature."""

    def test_signature_has_pg_files_device(self):
        """Constructor should accept (pg, hf_weights_files, device)."""
        import inspect

        sig = inspect.signature(AutoLoader.__init__)
...
class TestAutoLoaderBaseLoader:
    """Test AutoLoader creates SafeTensorsFileLoader for loader='base'."""
    ...
    def test_default_creates_base_loader(self, monkeypatch):
        ...
        with (
            ...
            unified = AutoLoader(None, ["file1.safetensors"], device="cpu")
    ...
    def test_base_no_extension_uses_defaults(self, monkeypatch):
        ...
        with (
            ...
            AutoLoader(None, ["file1.safetensors"], device="cpu")
...
class TestAutoLoader3FSLoader:
    """Test AutoLoader creates ThreeFSLoader for loader='3fs'."""
    ...
    def test_3fs_creates_threefs_loader(self, monkeypatch):
        ...
        with (
            patch("fastsafetensors.auto_loader.ThreeFSLoader") as mock_3fs,
            patch("fastsafetensors.auto_loader.PipelineParallel"),
            patch("fastsafetensors.auto_loader.load_config") as mock_load,
        ):
            ...
            unified = AutoLoader(None, ["file1.safetensors"], device="cuda:0")
...
class TestAutoLoaderUnknownLoader:
    """Test AutoLoader raises ValueError for unknown loader type."""

    def test_unknown_loader_raises(self):
        with patch("fastsafetensors.auto_loader.load_config") as mock_load:
            mock_load.return_value = LoaderConfig(loader="nonexistent")

            with pytest.raises(ValueError, match="Unknown loader type"):
                AutoLoader(None, ["file1.safetensors"])


class TestAutoLoaderPipelineCreation:
    """Test that AutoLoader creates PipelineParallel correctly."""

    def test_pipeline_receives_parallel_kwargs(self):
        """PipelineParallel should receive parallel kwargs from config."""
        with (
            patch("fastsafetensors.auto_loader.SafeTensorsFileLoader") as mock_stfl,
            patch("fastsafetensors.auto_loader.PipelineParallel") as mock_pp,
            patch("fastsafetensors.auto_loader.load_config") as mock_load,
        ):
...
class TestAutoLoaderConfigDiscovery:
    ...
    def test_env_var_config(self, monkeypatch):
        ...
        try:
            ...
            with (
                ...
                mock_3fs.return_value = MagicMock()
                unified = AutoLoader(None, ["file1.safetensors"])
                assert unified.config.loader == "3fs"
                assert (
    ...
    def test_no_config_uses_defaults(self, monkeypatch):
        ...
        monkeypatch.delenv(CONFIG_ENV_VAR, raising=False)
        # cd to a temp dir where no fastsafetensors.json exists
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                with (
                    patch(
                        "fastsafetensors.auto_loader.SafeTensorsFileLoader"
                    ) as mock_stfl,
                    patch("fastsafetensors.auto_loader.PipelineParallel"),
                ):
                    mock_stfl.return_value = MagicMock()
...
class TestAutoLoaderRemovedAPIs:
    ...
    def test_no_context_manager(self):
        assert not hasattr(AutoLoader, "__enter__")
        assert not hasattr(AutoLoader, "__exit__")

    def test_no_iter(self):
        # __iter__ should be removed; AutoLoader is not iterable.
        assert "__iter__" not in vars(AutoLoader)

    def test_no_loader_property(self):
        # The public ``loader`` property is removed; ``_loader`` remains
        # as an internal attribute only.
        assert "loader" not in vars(AutoLoader)


class TestAutoLoaderProperties:
    """Test AutoLoader properties."""
...
class TestAutoLoaderClose:
    """Verify AutoLoader.close() does not double-close the underlying loader.

    PipelineParallel.close() already closes its inner loader, so
    AutoLoader.close() must only call pipeline.close() and must NOT call
...
class TestProcessExtensionConfig:
    """Test that process_extension_config is correctly invoked via AutoLoader."""
...
class TestKwargsPassedToProcessExtensionConfig:
    """Verify hf_weights_files reaches process_extension_config via AutoLoader."""
    ...
    def test_hf_weights_files_passed_to_base(self):
        ...
        with (
            ...
            AutoLoader(None, ["a.safetensors", "b.safetensors"], device="cpu")
    ...
    def test_hf_weights_files_passed_to_3fs(self):
        ...
        with (
            ...
            AutoLoader(None, ["/data/3fs/m1.safetensors"], device="cuda:0")
...
class TestInitExports:
    ...
    def test_auto_loader_importable(self):
        """AutoLoader should be importable from top-level package."""
        from fastsafetensors import AutoLoader as AL

        assert AL is not None
        assert AL is AutoLoader
