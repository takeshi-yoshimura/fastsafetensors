...
class TestLoaderConfigFromFile:
    """Test LoaderConfig.from_file() (public entry point).

    Only JSON format is supported. ``_from_json`` is a private implementation
    detail; we verify through the public ``from_file``.
    """
    ...
    def test_from_file_json_3fs(self):
        data = {
            "loader": "3fs",
            "3fs": {"mount_point": "/data/3fs"},
            "parallel": {"max_concurrent_producers": 1},
        }
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump(data, f)
            f.flush()
    ...
    def test_from_file_json_empty(self):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump({}, f)
            f.flush()
            path = f.name
    ...
    def test_auto_detect_json_3fs_extension(self):
        data = {"loader": "3fs"}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump(data, f)
            f.flush()
...
class TestLoaderConfigPublicAPI:
    ...
    def test_yaml_support_removed(self):
        # Both public ``from_yaml`` and private ``_from_yaml`` should be removed.
        assert not hasattr(LoaderConfig, "from_yaml")
        assert not hasattr(LoaderConfig, "_from_yaml")
...
class TestLoadConfig:
    ...
    def test_no_config_uses_defaults(self, monkeypatch, tmp_path):
        ...
        (``./fastsafetensors.json``) is guaranteed to be absent, regardless of
